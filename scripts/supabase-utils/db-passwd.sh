...
# - Use .env.supabase for the admin user and database service port
...
if [ ! -f .env.supabase ]; then
    echo "Missing .env.supabase file. Exiting."
    exit 1
fi
...
db_srv_port=$(grep "^POSTGRES_PORT=" .env.supabase | cut -d '=' -f 2)
port_source=" (.env.supabase):"
...
echo "Updating POSTGRES_PASSWORD in .env.supabase..."
sed -i.old "s|^POSTGRES_PASSWORD=.*$|POSTGRES_PASSWORD=$new_passwd|" .env.supabase
