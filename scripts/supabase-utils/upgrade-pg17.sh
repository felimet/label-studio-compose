...
preflight() {
    ...
    [ -f .env.supabase ] || die "Missing .env.supabase file."
    ...
    pg_password=$(grep '^POSTGRES_PASSWORD=' .env.supabase | cut -d '=' -f 2- | sed "s/^['\"]//;s/['\"]$//" | head -n 1)
    [ -n "$pg_password" ] || die "POSTGRES_PASSWORD not set in .env.supabase."
