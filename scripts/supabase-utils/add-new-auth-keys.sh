...
#
# Reads JWT_SECRET from .env.supabase and generates:
...
#
# Usage:
#   sh add-new-auth-keys.sh              # Interactive: prints keys, prompts to update .env.supabase
#   sh add-new-auth-keys.sh --update-env # Prints keys and writes them to .env.supabase
#   sh add-new-auth-keys.sh | tee keys   # Non-interactive: prints keys only
#
# Prerequisites:
#   - .env.supabase file with JWT_SECRET set (run generate-keys.sh first)
#   - openssl
#   - node >= 16
#
...
# Read JWT_SECRET from .env.supabase
if [ ! -f .env.supabase ]; then
    echo "Error: .env.supabase file not found. Run generate-keys.sh first."
    exit 1
fi

jwt_secret=$(grep '^JWT_SECRET=' .env.supabase | cut -d= -f2-)
if [ -z "$jwt_secret" ]; then
    echo "Error: JWT_SECRET not found in .env.supabase. Run generate-keys.sh first."
    exit 1
fi
...
if [ "$1" = "--update-env" ]; then
    ...
    printf "Update .env.supabase file? (y/N) "
    read -r REPLY
    case "$REPLY" in
        [Yy]) update_env=true ;;
        *) update_env=false ;;
    ...
    echo "Running non-interactively. Pass --update-env to write to .env.supabase."
    update_env=false
fi
...
for var in SUPABASE_PUBLISHABLE_KEY SUPABASE_SECRET_KEY ANON_KEY_ASYMMETRIC SERVICE_ROLE_KEY_ASYMMETRIC JWT_KEYS JWT_JWKS; do
    eval "val=\$$var"
    if grep -q "^${var}=" .env.supabase; then
        sed -i.old -e "s|^${var}=.*$|${var}=${val}|" .env.supabase
    else
        echo "${var}=${val}" >> .env.supabase
    fi
done
