...
| 變數 | 預設值 | 說明 |
|------|--------|------|
| `PG_DB_IMAGE` | `supabase/postgres:15.14.1.107` | `pg-db` 映像（v1.1.0 預設為 Supabase Postgres runtime） |
...
- `supabase-storage` / `supabase-imgproxy`：S3 profile 示範延伸（`supabase-s3`，非本版運作流程）
...
### 示例延伸：`supabase-s3` profile（Storage + imgproxy，非本版流程）
...
## Supabase 獨立管理模式（`.env.supabase` + `docker-compose.supabase.yml`，本分支預設）

若需求是「使用 Supabase 進行管理，且不使用原生 `pg-db`」，請使用官方獨立 stack：
...
make supabase-standalone-up SUPABASE_STANDALONE_ENV=.env.supabase
```
